...
class Structure extends CI_Controller {
	...
	{
		$data['cur_page'] = 'structure';
		$data['cur_parent_page'] = 'about_company';
		$data['datas'] = $this->M_Structure->show_structure();
		$this->load->view('admin/module/structure/index', $data);
	}
	...
	public function create()
	{
		...
		$config['upload_path']          = FCPATH.'assets/uploads/structure';
        ...
        if ($_FILES["banner_img"]['name'] == '' || $_FILES["banner_img"]['name'] == NULL ) {
        	if ($id == 'empty') {
		        ...
		        $insert = $this->M_Structure->insert($datas,'t_image_profile');
			}else{
				$datas = array(
		        	'id' => $id,
		        	'type' => 'structure',
				...
				$insert = $this->M_Structure->update($datas, $id);
			}
	        if ($insert) {
	        	$this->session->set_flashdata('title','Success');
	        	$this->session->set_flashdata('message','Update data structure');
	        	$this->session->set_flashdata('status','success');
	        	redirect('/administrator/structure');
	        }else{
        	...
        	if ( ! $this->upload->do_upload('banner_img'))
	        {
	        	...
	        	$this->session->set_flashdata('status','error');
	        	redirect('/administrator/structure/index');
	        }
	        ...
	        else
	        {
		        ...
		        if ($id == 'empty') {
					...
					$result = $this->M_Structure->show_structure();
					if ($result) {
