...
class Doctor extends CI_Controller {
    ...
    public function detail($id, $name)
    {
        ...
        if (!is_numeric($id) || $id <= 0 || empty($id) || !is_string($name) || empty($name)) {
        ...
        if (!$doctor_details) {
            ...
            show_error('Doctor not found'.var_dump($id));
            return;
        }
        ...
        $data['seo_url'] = base_url().'doctor-'.$doctor_details[0]->id.'-'.$this->_generateSlug($doctor_details[0]->name).'.html';
    ...
    // Helper function to generate slug from doctor's name
    private function _generateSlug($name) {
        $lower_name = strtolower($name);
        $delete_special_chars = preg_replace('/[^a-zA-Z0-9\s]/', '', $lower_name);
        $replace_spaces_with_dashes = str_replace(' ', '-', $delete_special_chars);
        return $replace_spaces_with_dashes;
    }
